# Based on SenseHatB code from https://www.waveshare.com/wiki/Sense_HAT_(B)
import time
import smbus
#i2c address
...
class ADS1015(object):
    ...
    def read(self,channel):                    #Read single channel data
        data=0
        ADS_Config_Set =  ( ADS_CONFIG_MODE_NOCONTINUOUS        |   #mode：Single-shot mode or power-down state    (default)
        ...
        if channel == 0:
            ADS_Config_Set |= ADS_CONFIG_MUX_SINGLE_0
        elif channel == 1:
            ADS_Config_Set |= ADS_CONFIG_MUX_SINGLE_1
        elif channel == 2:
            ADS_Config_Set |= ADS_CONFIG_MUX_SINGLE_2
        elif channel == 3:
            ADS_Config_Set |= ADS_CONFIG_MUX_SINGLE_3
        ADS_Config_Set |=ADS_CONFIG_OS_SINGLE_CONVERT
        self._write_word(ADS_POINTER_CONFIG,ADS_Config_Set)
        time.sleep(0.01)
